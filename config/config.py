from Env.UciMapping import UciMapping

# Multiprocess
NUM_WORKERS = 24
MIN_EVALUATE_COUNT = 16

# Network type
NETWORK_TYPE = 'attention' # attention/cnn

# Pretrain
USING_PRETRAIN = True
NO_PRETRAIN_LOSER = True
PRETRAIN_FILE = 'pretrain_data/pretrain.pgn'
OPENING_FILE = 'pretrain_data/opening.pgn'
PRETRAIN_GAME_ITERATION = 1000
PRETRAIN_EPOCHS = 2
PRETRAIN_VALIDATION_SPLIT = 0.01
PRETRAIN_VAL_STEP = 20
PRETRAIN_SKIP_OPENING_STEP = 10
PRETRAIN_WARM_UP_RATIO = 0.1
LABEL_SMOOTHING = 0

# MCTS
BATCH_SIZE = 256
GAME_TRAIN_STEP = 500
MAX_THINK_LOOP = 4
KLD_THRESHOLD = 0.1
EXP_MIN = 10000
EXP_MAX = 150000
RESIGN_PERCENTAGE = 2.0
RESIGN_PLAYTHROUGH = 20
MIN_RESIGN_TURN = 5
WIN_UPDATE_PERCENT = 0.55
FPU_VALUE_AT_ROOT = 1
SMART_PRUNING_FACTOR = 1.33
BONUS_END_POSITION = 0.05
VIRTUAL_LOSS = 1.0
INF = 1e9

ROOT_EXPLORATION_WEIGHT = 2.0

PLAY_THREAD = 32

TEMPERATURE = 0.9
TEMPERATURE_DELAY = 20
TEMPERATURE_DECAY_MOVE = 60
TEMPERATURE_ENDGAME = 0.6
TEMPERATURE_DECAY = TEMPERATURE / TEMPERATURE_DECAY_MOVE # linear
TEMPERATURE_VISIT_OFFSET = -0.8
GAME_EVALUATE = 50

# Dirichlet noise
DIRICHLET_ALPHA = 0.12
DIRICHLET_EPSILON = 0.1

SAVE_MODEL_PATH = 'saved_model/'
MODEL_NAME = 'chess_checkpoint.pth'
MODEL_ONNX_NAME = 'chess_model.onnx'
TABLEBASE_PATH = "Syzygy/"
GAVIOTA_PATH = "Gaviota"

# Optimizer
LEARNING_RATE = 0.02
STOP_LEARNING_RATE = 4e-6
MOMENTUM = 0.9
L2_CONST = 1e-4
MAX_GRAD_NORM = 3.0
DECAY_RATE = 0.1
UPDATE_LR_STEP = 5
LOSE_WEIGHTS = [1.0, 1.0] # policy, value

# Environment
EPISODE = 1000000

# Socket
host = '127.0.0.1'
port = 12345

LABELS_MAP = UciMapping(NETWORK_TYPE)